...
2、交互过程必须以简体中文进行展示
3、使用下面代码助力完善项目
完整 Kotlin 代码（一个文件即可，例如 RapidCompiler.kt）
...
data class Param(
    val typeName: String,
    val name: String,
    val byRef: Boolean,  // 这里预留，RAPID 有 REF 相关机制
    val spanParam: Span
)
...
data class IfStmt(
    val branches: List<IfBranch>,          // IF + 若干 ELSEIF
    val elseBranch: List<StmtNode>?,       // ELSE
    override val span: Span
) : StmtNode
...
class Parser(private val tokens: List<Token>) {
    ...
    private fun parsePrimary(): ExprNode {
        ...
        return when (tok.type) {
            ...
            TokenType.IDENT -> parseIdentOrCallOrAccess()
            TokenType.LPAREN -> {
                advance()
    ...
    private fun parseIdentOrCallOrAccess(): ExprNode {
        ...
        loop@ while (true) {
            when {
                ...
                match(TokenType.DOT) -> {
                    // 如果需要支持 a.b 形式，可开启这个逻辑
                    // 当前 TokenType 中未定义 DOT，如要用，需在 TokenType 中补上 DOT
                    // 这里为了简洁，暂不实现
                    break@loop
                }
                else -> break@loop
...
data class Symbol(
    val name: String,
    val type: String?,        // VAR: 类型名，PROC: null，FUNC: 返回类型
    val isConst: Boolean,
    val decl: AstNode
)
    ...
    private fun analyzeModule(module: ModuleNode, parent: Scope) {
        ...
        module.declarations.forEach { decl ->
            when (decl) {
                ...
                is ProcDecl -> {
                    val ok = scope.define(
                        Symbol(decl.name, null, false, decl)
                    )
                    if (!ok) report("重复定义 PROC: ${decl.name}", decl.span)
                }
                is FuncDecl -> {
                    val ok = scope.define(
                        Symbol(decl.name, decl.returnType, false, decl)
                    )
        ...
        module.declarations.forEach {
            when (it) {
                is VarDecl -> {
                    it.initExpr?.let { e -> analyzeExpr(e, scope) }
                }
        ...
        return when (expr) {
            ...
            is CallExpr -> {
                ...
                if (sym == null) {
                    ...
                    sym.type // FUNC 返回类型；PROC 为 null
                }
            }
            is UnaryExpr -> {
                val t = analyzeExpr(expr.expr, scope)
                t
            }
            is BinaryExpr -> {
...
1. RapidViewModel（负责调用 RapidCompiler）

package com.yourcompany.rapidviewer
...
class RapidViewModel : ViewModel() {
    ...
    private val _diagnostics = MutableStateFlow(emptyList<Diagnostic>())
    ...
    private fun runCheck(src: String) {
        viewModelScope.launch(Dispatchers.Default) {
            val result = RapidCompiler.analyze(src)
            _diagnostics.value = result.diagnostics
            _program.value = result.program
        }
    }
}


2. Jetpack Compose 编辑器 + 诊断列表 UI

package com.yourcompany.rapidviewer.ui
...
import com.yourcompany.rapidviewer.*

@Composable
fun RapidEditorScreen(vm: RapidViewModel) {
    ...
    Column(
        ...
        Text("RAPID 程序代码", style = MaterialTheme.typography.titleMedium)
...
3. DiagnosticsList：错误/警告列表

@Composable
fun DiagnosticsList(diags: List<Diagnostic>) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
        ...
        Text("语法检查结果：", style = MaterialTheme.typography.titleMedium)

        if (diags.isEmpty()) {
            Text("✔ 无错误", color = Color(0xFF008800))
        } else {
            diags.forEach { d ->
                val color = when (d.severity) {
                    Severity.ERROR -> Color.Red
                    Severity.WARNING -> Color(0xFFFFA500)
...
放进现有 RapidCompiler.kt 的完整 Kotlin 代码补丁

第一部分：AST 扩展（直接复制到 AST 区域）

// =============== 新增：TEST 结构 ===============

data class TestCase(
    val values: List<ExprNode>,   // CASE 1,2,3
    val body: List<StmtNode>,
    val span: Span
)

data class TestStmt(
    val expr: ExprNode,                 // TEST <expr>
    val cases: List<TestCase>,          // CASE ...
    val defaultBody: List<StmtNode>?,   // DEFAULT
    override val span: Span
) : StmtNode
...
// =============== 新增：CONNECT 指令 （CONNECT trap WITH err） ===============
...
'.' -> {
    advance()
    tokens.add(Token(TokenType.DOT, ".", makeSpan(startLine, startCol)))
}
...
private fun parseRecordDecl(): RecordDecl {
    ...
    return RecordDecl(
        nameTok.lexeme,
        fields,
        mergeSpan(recTok.span, semi.span)
    )
}
...
private fun parseTestStmt(): TestStmt {
    ...
    while (true) {
        when {
            match(TokenType.CASE) -> {
                ...
                val body = parseStmtBlockUntil(
                    setOf(TokenType.CASE, TokenType.DEFAULT, TokenType.ENDTEST)
                )
    ...
    return TestStmt(
        ...
        cases,
        defaultBody,
        mergeSpan(testTok.span, endTok.span)
    )
}

在 parseStatement() 中加入：
...
第四部分：语义检查扩展（SemanticAnalyzer）
① 注册 RECORD 类型
在 analyzeModule 中：
is RecordDecl -> {
    val ok = scope.define(Symbol(it.name, "record", false, it))
    if (!ok) report("重复定义 RECORD: ${it.name}", it.span)
}
...
is DotAccess -> {
    ...
    if (decl !is RecordDecl) {
        report("类型 $baseType 不是 RECORD，不能使用点访问", expr.span)
        return null
    }
...
第五部分：新增 RAPID 内置类型（工具/速度/工件）

在 SemanticAnalyzer 中添加：
