 ...
 * and record a diagnostic when a close match is found.
 */
private fun addIncompleteKeywordDiagnostic(
    word: String,
    startLine: Int,
    startCol: Int,
    ...
    val knownInstructions = setOf(
        ...
        "IsDnum", "IsNum", "IsBoolean", "IsString",
        "StrLen", "StrMatch", "StrToVal", "ValToStr",
        "ClearPath", "CalcJointT", "CalcRobT",
        "DeactPos", "ActPos",
        "IPRead", "IPWrite",
        "SocketCreate", "SocketConnect", "SocketClose", "SocketSend", "SocketReceive"
    )

    // If it's already a known keyword or instruction, no need to warn
    if (knownKeywords.contains(word.uppercase()) || knownInstructions.contains(word)) {
        return
    }

    // Find closest match among known keywords and instructions
    var closestMatch: String? = null
    var minDistance = Int.MAX_VALUE

    fun considerCandidates(candidates: Set<String>) {
    ...
    considerCandidates(knownKeywords)
    considerCandidates(knownInstructions)

    // Report warning if a close match is found
    if (closestMatch != null && (minDistance <= 2 || minDistance == 0)) {
...
class Parser(private val tokens: List<Token>) {
    ...
    private fun checkStatementLevelIdent(name: String, span: Span) {
        ...
        addIncompleteKeywordDiagnostic(name, span.startLine, span.startCol, diagnostics)
    }
